...
PathToDir="$HOME""/programs_setup/opensim"
...
##for opensim 0.9.1
echo "[OSSL]" | tee -a $PathToDir/opensim-*/bin/config-include/StandaloneCommon.ini
echo " AllowOSFunctions = true" | tee -a $PathToDir/opensim-*/bin/config-include/StandaloneCommon.ini
echo "OSFunctionThreatLevel = Severe" | tee -a $PathToDir/opensim-*/bin/config-include/StandaloneCommon.ini
echo " Allow_osConsoleCommand = $YOUR_LONG_ID" | tee -a $PathToDir/opensim-*/bin/config-include/StandaloneCommon.ini
echo "[NPC]" | tee -a $PathToDir/opensim-*/bin/config-include/StandaloneCommon.ini
echo "Enabled = true" | tee -a $PathToDir/opensim-*/bin/config-include/StandaloneCommon.ini
